...
import { Card, CardContent, CardHeader, CardTitle } from '@flama/design-system-mobile/card';
...
export default function WalletScreen() {
  ...
  const reset = useResetWallet({
    onSuccess: () => router.replace('/(app)/wallet/import'),
  });
  ...
  if (status === 'no_wallet') {
    return <Redirect href="/(app)/wallet/import" />;
  }
  ...
  return (
    <>
      ...
      <ScrollView contentContainerClassName="p-6 gap-6">
        ...
        <Button variant="destructive" onPress={() => reset.mutate()} disabled={reset.isPending}>
      ...
      <CardContent className="gap-4">
        ...
        <View className="flex-row gap-3">
          <Button
            ...
            onPress={() =>
              router.push({
                pathname: '/(app)/wallet/send',
                params: { chainId: account.chainId },
              })
            }
          >
