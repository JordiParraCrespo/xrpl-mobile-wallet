...
export default function SendScreen() {
  ...
  if (!account) {
    return <Redirect href={Routes.Wallet} />;
  }
  ...
  return (
    <>
      <Stack.Screen
        options={{ title: t("wallet.send.title", { symbol: account.symbol }) }}
      />
      <ScrollView contentContainerClassName="p-6 gap-6">
        <View className="gap-2">
          ...
          {balance.data && (
            <Text className="text-sm text-muted-foreground">
              {t("wallet.accounts.balance")}: {balance.data.formatted}{" "}
              {account.symbol}
            </Text>
          )}
        </View>
        ...
        {send.isError && (
          <Text className="text-sm text-destructive">{send.error.message}</Text>
        )}
        ...
        {result && (
          <View className="gap-2">
            ...
            >
              {result.success
                ? t("wallet.send.success")
                : t("wallet.send.failure")}
            ...
            {result.explorerUrl && (
              <Button
                variant="outline"
                onPress={() =>
                  result.explorerUrl && Linking.openURL(result.explorerUrl)
                }
              >
        ...
        <Button
          onPress={() =>
            send.mutate({ chainId: account.chainId, to: to.trim(), amount })
          }
