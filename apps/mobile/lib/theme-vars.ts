// Drops design tokens as NativeWind CSS variable maps — the single source the
// root layout feeds to `vars()` so the whole tree resolves `bg-background`,
// `text-foreground`, `bg-brand`, etc. Keep in sync with global.css (the web
// `:root` / `.dark:root` blocks).
//
// Exported so off-theme surfaces can pin a token context explicitly: the
// full-screen money flows (add money · receive · swap · send) are always dark
// regardless of the app's color scheme, so they wrap their root in
// `vars(DARK_THEME_VARS)` to keep the shared dark-surface primitives (Keypad,
// Button, detail rows) rendering light-on-dark.
...
/** The flow surfaces paint on a near-black base, darker than the app's dark
 * `--background`, matching the Drops money-flow design (`flow-kit.jsx`). */
